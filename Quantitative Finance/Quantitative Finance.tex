...
% Behavioral Finance - mispricing study
\newpage
    \begin{center}
...
\newpage
    ...
    \begin{itemize}
        \item 
        $P_{0}+S_{0}=C_{0}+P V(K)$\\
        (In other words, bonds that par at K)

        \item Graph representation of the replication\\
            Page 17
        \item 

        \begin{tabular}{|l|l|}
        \hline$P_{0}+S_{0}=C_{0}+PV(K)$ & \multicolumn{1}{|c|} { Protective Put} \\
        \hline$P_{0}=C_{0}+P V(K)-S_{0}$ & \multicolumn{1}{|c|} { Synthetic Put }  \\
        \hline$C_{0}=P_{0}+S_{0}-P V(K)$ & Synthetic Cal \\
        \hline$S_{0}=C_{0}-P_{0}+P V(K)$ & Synthetic Stock \\
        \hline$P V(K)=S_{0}+P_{0}-C_{0}$ & Synthetic Bond \\
        \hline
        \end{tabular}
        \item Further discussion:\\
        1. if a stock paydividend : substract PV(divident) from stock value
    \end{itemize}

    \section{Option Strategies}
    \begin{itemize}
        \item Speculate events or hedge risk
        \item synthesize option payoff
        \item Arbitrage: option and synthetic option prices are close
    \end{itemize}

    \section{It is all about {\color{red}Volatility}}
    \begin{itemize}
        \item Implied Volatility: volatility value that makes option value equals actual market price
        \item Average of 30-day and 1-year historical non-earnings volatility
    \end{itemize}

    \section{Black-Schole's Formulation}
    \begin{itemize}
        \item Assume log-normal underlying asset price
        \item Assume constant volatility
        \item all options based on the underlying asset, the implied volatility should be the same
    \end{itemize}

    \section{Implied Volatility Property}
    \begin{itemize}
        \item Term structure of volatility (Maturity)
        \item Stochastic volatility
        \item Differ by strike price\\
        1. Skew\\
        2. Smile: extreme strike price have high implied volatility
        3. Smirk
    \end{itemize}
